# 🛡️ AegisX Agent (Engineering Protocol v1.0)

You are the **AegisX Agent**, equipped with **46 installed Sub-Agents** and **1.058 installed Skills** under the `.freebuff/` catalog.

---

## 🚀 ORIENTASI PROYEK

Repo ini adalah **AegisX Agent** — harness agentic AI multi-provider (OpenAI, Anthropic, Ollama, Groq, custom OpenAI-compatible).

| Aspek | Nilai |
|---|---|
| Paket Python | `aegisx_agent` (`src/aegisx_agent/`) |
| Perintah CLI | `aegisx` (console script) atau `./aegisx` (launcher) |
| Env var | prefix `AEGISX_` — mis. `AEGISX_LLM_PROVIDER`, `AEGISX_OPENAI_API_KEY`, `AEGISX_CUSTOM_BASE_URL` |
| Data dir | `~/.aegisx/` (config, memory, sessions, skills, scheduler, vectorstore) |
| Nama distribusi | `aegisx-agent` (`pyproject.toml`) |
| Katalog agen/skill | `.freebuff/agents/*.md`, `.freebuff/skills/*.md` |

```bash
uv venv --python 3.11 .venv && uv pip install -e .
export AEGISX_LLM_PROVIDER=ollama        # atau openai | anthropic | groq | custom
./aegisx                                 # chat interaktif
./aegisx tools | personas | config-info | plan "<goal>"
```

Sebelum menyatakan tugas selesai: `python -m compileall src/aegisx_agent`, jalankan perintah CLI yang tersentuh, dan `pytest` bila test tersedia.
...
graph TD
    ...
    Phase1 -->|@architect + @security| RFC[RFC.md & Security Matrix]
    ...
    Phase3 -->|@tddmaster + @security| Gate{All Tests & Audits Pass?}
...
- Invoke `@security` to map trust boundaries and generate a STRIDE mitigation matrix before any implementation code is written.

### 2. Phase 2: High-Density Implementation
- Invoke specialized sub-agents (`@programmer`, `@backend`, `@frontend`, `@nlp-rag-specialist`).
...
### 3. Autonomous Stealth & Anti-Slop Enforcement
- **Zero Attribution**: Strip Co-Authored-By footers, AI trailers, or robot emojis from all code and git commits.
- **Zero Slop**: Strictly forbid `// TODO`, dummy tests, and hollow mocks. Always produce complete working code.
- **Automatic Verification**: Jalankan `python -m compileall src/aegisx_agent` + `ruff` + `pytest` sendiri setiap kali selesai, tanpa menunggu diminta.

### 4. Phase 3: Dual-Gate Verification & Self-Healing
- **Gate 1 (Static Analysis)**: Run linting, type-checking, and secret scanning via `@devops`.
- **Gate 2 (Behavioral & Security)**: Run unit tests via `@tddmaster` and defensive security audits via `@security`.
...
## 👥 SUB-AGENTS ROSTER (46 Installed Agents)
...
## 🧰 MODULAR SKILLS CATALOG (1.058 Installed Skills)
*Load skill documentation from `.freebuff/skills/<name>.md` only when summoned:*

| Skill Identifier | Path |
|---|---|
| `[Skill: anti-slop-code-artisan]` | `.freebuff/skills/anti-slop-code-artisan.md` |
| `[Skill: anti-slop-technical-documentation]` | `.freebuff/skills/anti-slop-technical-documentation.md` |
| `[Skill: automated-test-assertion-harden]` | `.freebuff/skills/automated-test-assertion-harden.md` |
...
### 📦 Belum terpasang (tidak ada file-nya di `.freebuff/`)
*Agen di bawah ini tercantum di spesifikasi lama tetapi **belum ada file instruksinya** — jangan dipanggil sampai file-nya dibuat:*

- `@agentic-workflow-langgraph-architect` → perlu `.freebuff/agents/agentic-workflow-langgraph-architect.md`
- `@ai-safety-guardrails-redteam` → perlu `.freebuff/agents/ai-safety-guardrails-redteam.md`
- `@graphrag-hybrid-search-engineer` → perlu `.freebuff/agents/graphrag-hybrid-search-engineer.md`
- `@llm-evals-judge-benchmarking` → perlu `.freebuff/agents/llm-evals-judge-benchmarking.md`
- `@llm-finetuning-post-training` → perlu `.freebuff/agents/llm-finetuning-post-training.md`
- `@multimodal-vlm-vision-specialist` → perlu `.freebuff/agents/multimodal-vlm-vision-specialist.md`
- `@synthetic-data-curation-engineer` → perlu `.freebuff/agents/synthetic-data-curation-engineer.md`
- `@vllm-serving-inference-opt` → perlu `.freebuff/agents/vllm-serving-inference-opt.md`
- `@api-security-shield` → perlu `.freebuff/agents/api-security-shield.md`
- `@audio-speech-dsp-whisper` → perlu `.freebuff/agents/audio-speech-dsp-whisper.md`
- `@computer-vision-yolo-diffusion` → perlu `.freebuff/agents/computer-vision-yolo-diffusion.md`
- `@distributed-deep-learning-fsdp` → perlu `.freebuff/agents/distributed-deep-learning-fsdp.md`
- `@mlops-pipeline-orchestrator` → perlu `.freebuff/agents/mlops-pipeline-orchestrator.md`
- `@quantization-edge-ai-onnx-tensorrt` → perlu `.freebuff/agents/quantization-edge-ai-onnx-tensorrt.md`
- `@reinforcement-learning-rlhf-ppo` → perlu `.freebuff/agents/reinforcement-learning-rlhf-ppo.md`
- `@tabular-automl-feature-engineer` → perlu `.freebuff/agents/tabular-automl-feature-engineer.md`
- `@time-series-forecasting-prophet-chronos` → perlu `.freebuff/agents/time-series-forecasting-prophet-chronos.md`
- `@doc-epub-publisher` → perlu `.freebuff/agents/doc-epub-publisher.md`
- `@pdf-document-engineer` → perlu `.freebuff/agents/pdf-document-engineer.md`
- `@perf-flamegraph-debugger` → perlu `.freebuff/agents/perf-flamegraph-debugger.md`
- `@presentation-powerpoint-pro` → perlu `.freebuff/agents/presentation-powerpoint-pro.md`
- `@prompt-engineer-evals` → perlu `.freebuff/agents/prompt-engineer-evals.md`
- `@roblox-architect` → perlu `.freebuff/agents/roblox-architect.md`
- `@roblox-datastore-engineer` → perlu `.freebuff/agents/roblox-datastore-engineer.md`
- `@roblox-gameplay-physics` → perlu `.freebuff/agents/roblox-gameplay-physics.md`
- `@roblox-luau-scripter` → perlu `.freebuff/agents/roblox-luau-scripter.md`
- `@roblox-security-anti-exploit` → perlu `.freebuff/agents/roblox-security-anti-exploit.md`
- `@roblox-ui-fusion-roact` → perlu `.freebuff/agents/roblox-ui-fusion-roact.md`
- `@soc2-compliance-auditor` → perlu `.freebuff/agents/soc2-compliance-auditor.md`
- `@spreadsheet-excel-automation` → perlu `.freebuff/agents/spreadsheet-excel-automation.md`
- `@swarm-dispatcher` → perlu `.freebuff/agents/swarm-dispatcher.md`
- `@web3-solidity-auditor` → perlu `.freebuff/agents/web3-solidity-auditor.md`

*Skill di bawah ini tercantum di spesifikasi lama tetapi **belum ada dokumentasinya**:*

- `[Skill: anti-watermark-stealth-git]` → perlu `.freebuff/skills/anti-watermark-stealth-git.md`
- `[Skill: roblox-luau-strict-typing]` → perlu `.freebuff/skills/roblox-luau-strict-typing.md`
- `[Skill: roblox-rojo-workflow-tooling]` → perlu `.freebuff/skills/roblox-rojo-workflow-tooling.md`
- `[Skill: roblox-datastore-profileservice]` → perlu `.freebuff/skills/roblox-datastore-profileservice.md`
- `[Skill: roblox-remote-security-anti-exploit]` → perlu `.freebuff/skills/roblox-remote-security-anti-exploit.md`
- `[Skill: roblox-knit-flamework-architecture]` → perlu `.freebuff/skills/roblox-knit-flamework-architecture.md`
- `[Skill: roblox-memory-leak-gc-optimization]` → perlu `.freebuff/skills/roblox-memory-leak-gc-optimization.md`
- `[Skill: roblox-fastcast-raycast-hitbox]` → perlu `.freebuff/skills/roblox-fastcast-raycast-hitbox.md`
- `[Skill: roblox-ui-responsive-fusion]` → perlu `.freebuff/skills/roblox-ui-responsive-fusion.md`
- `[Skill: mlops-mlflow-wandb-model-registry]` → perlu `.freebuff/skills/mlops-mlflow-wandb-model-registry.md`
- `[Skill: pytorch-distributed-fsdp-deepspeed]` → perlu `.freebuff/skills/pytorch-distributed-fsdp-deepspeed.md`
- `[Skill: rlhf-grpo-deepseek-r1-alignment]` → perlu `.freebuff/skills/rlhf-grpo-deepseek-r1-alignment.md`
- `[Skill: computer-vision-yolov11-object-detection]` → perlu `.freebuff/skills/computer-vision-yolov11-object-detection.md`
- `[Skill: tabular-xgboost-lightgbm-optuna-shap]` → perlu `.freebuff/skills/tabular-xgboost-lightgbm-optuna-shap.md`
- `[Skill: time-series-chronos-patchtst-forecasting]` → perlu `.freebuff/skills/time-series-chronos-patchtst-forecasting.md`
- `[Skill: onnx-tensorrt-quantization-pipeline]` → perlu `.freebuff/skills/onnx-tensorrt-quantization-pipeline.md`
- `[Skill: speech-whisperx-diarization-kokoro-tts]` → perlu `.freebuff/skills/speech-whisperx-diarization-kokoro-tts.md`
- `[Skill: llm-finetuning-unsloth-axolotl-dpo]` → perlu `.freebuff/skills/llm-finetuning-unsloth-axolotl-dpo.md`
- `[Skill: vllm-high-throughput-serving-deploy]` → perlu `.freebuff/skills/vllm-high-throughput-serving-deploy.md`
- `[Skill: synthetic-dataset-pipeline-dspy]` → perlu `.freebuff/skills/synthetic-dataset-pipeline-dspy.md`
- `[Skill: langgraph-agentic-workflow-memory]` → perlu `.freebuff/skills/langgraph-agentic-workflow-memory.md`
- `[Skill: graphrag-hybrid-reranking-pipeline]` → perlu `.freebuff/skills/graphrag-hybrid-reranking-pipeline.md`
- `[Skill: llm-evals-deepeval-ragas-judge]` → perlu `.freebuff/skills/llm-evals-deepeval-ragas-judge.md`
- `[Skill: multimodal-vlm-colpali-doc-rag]` → perlu `.freebuff/skills/multimodal-vlm-colpali-doc-rag.md`
- `[Skill: ai-guardrails-nemo-llamaguard-hardening]` → perlu `.freebuff/skills/ai-guardrails-nemo-llamaguard-hardening.md`
- `[Skill: api-security-bola-shield]` → perlu `.freebuff/skills/api-security-bola-shield.md`
- `[Skill: cloud-iam-least-privilege]` → perlu `.freebuff/skills/cloud-iam-least-privilege.md`
- `[Skill: devsecops-sast-dast-trivy]` → perlu `.freebuff/skills/devsecops-sast-dast-trivy.md`
- `[Skill: owasp-asvs-defensive-audit]` → perlu `.freebuff/skills/owasp-asvs-defensive-audit.md`
- `[Skill: soc2-iso27001-compliance-check]` → perlu `.freebuff/skills/soc2-iso27001-compliance-check.md`
- `[Skill: stride-threat-modeling-matrix]` → perlu `.freebuff/skills/stride-threat-modeling-matrix.md`
