...
#: Reading history is safe; committing and checking out rewrite state.
_ACTION_RISKS: dict[str, ToolRisk] = {
    "status": ToolRisk.SAFE,
    "diff": ToolRisk.SAFE,
    "log": ToolRisk.SAFE,
    "add": ToolRisk.CAUTION,
    "stash": ToolRisk.CAUTION,
    "commit": ToolRisk.DANGEROUS,
    "checkout": ToolRisk.DANGEROUS,
}
...
class GitTool(Tool):
    ...
    def risk_for(self, arguments: dict[str, Any]) -> ToolRisk:
        """``branch`` only lists when no name is given, so it starts safe."""
        action = str(arguments.get("action", "status")).lower()
        if action == "branch":
            return ToolRisk.CAUTION if arguments.get("branch") else ToolRisk.SAFE
