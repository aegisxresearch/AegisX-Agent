...
#: Statements that only read. Anything else (or anything unrecognised) is
#: treated as a write, so a new SQL verb can never slip past unapproved.
_READ_ONLY_KEYWORDS = frozenset(
    {"select", "with", "explain", "pragma", "show", "describe", "desc", "values"}
)
_METADATA_ACTIONS = frozenset({"tables", "schema", "stats"})


def _statement_head(query: str) -> str:
    """First keyword of a SQL statement, or '' when there is none."""
...
class DatabaseQueryTool(Tool):
    ...
    def risk_for(self, arguments: dict[str, Any]) -> ToolRisk:
        """Read-only SQL is safe; every mutation needs approval.

        Multi-statement scripts are escalated because this tool runs them
        through ``executescript``: a harmless leading ``SELECT`` must not be
        able to smuggle a ``DROP`` along with it.
        """
        action = str(arguments.get("action", "query")).lower()
        ...
        if _statement_head(query) in _READ_ONLY_KEYWORDS:
            return ToolRisk.SAFE
        return ToolRisk.DANGEROUS
